...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.character-roles.update", [$characterRole->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('role') ? 'is-invalid' : '' }}" type="text" name="role" id="role" value="{{ old('role', $characterRole->role) }}" required>
                @if($errors->has('role'))
                    <div class="invalid-feedback">
                        {{ $errors->first('role') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('prompt') ? 'is-invalid' : '' }}" name="prompt" id="prompt" required>{{ old('prompt', $characterRole->prompt) }}</textarea>
