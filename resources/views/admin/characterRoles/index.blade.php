...
@can('character_role_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.character-roles.create') }}">
                        ...
                        <tr data-entry-id="{{ $characterRole->id }}">
                            ...
                            <td>
                                @can('character_role_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.character-roles.show', $characterRole->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('character_role_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.character-roles.edit', $characterRole->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('character_role_delete')
                                    <form action="{{ route('admin.character-roles.destroy', $characterRole->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('character_role_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.character-roles.massDestroy') }}",
