                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label class="required" for="character_id">{{ trans('cruds.task.fields.character') }}</label>
                            <select class="form-control select2" name="character_id" id="character_id" required>
                                @foreach($characters as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('character_id') ? old('character_id') : $task->character->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.task.fields.character_helper') }}</span>
                        </div>
                        <div class="form-group">
                            <label for="result">{{ trans('cruds.task.fields.result') }}</label>
                            <textarea class="form-control ckeditor" name="result" id="result">{!! old('result', $task->result) !!}</textarea>
                            @if($errors->has('result'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('result') }}
                                </div>
                            @endif
                            <span class="help-block">{{ trans('cruds.task.fields.result_helper') }}</span>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="status" id="status" value="{{ old('status', $task->status) }}">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('frontend.tasks.storeCKEditorImages') }}', true);
